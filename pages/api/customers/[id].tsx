...
export const editCustomer = async(id: string | ObjectId, customer: Customer) => {
    ...
    .db().collection('customers').replaceOne({_id: id}, customer)
...
export const deleteCustomer = async(id: string | ObjectId) => {
    ...
    return await mongoClient
    .db().collection('customers').deleteOne({_id: id});
}

export default async (
    req: NextApiRequest, 
    res: NextApiResponse<{modifiedCount: number} | {customer : Customer} | {error: string} | {deletedCount: number}>
    ) => {
